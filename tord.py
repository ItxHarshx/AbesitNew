...
def _player_list_text(state: dict) -> str:
    lines = [f"{i + 1}. {html.escape(p['name'])}" for i, p in enumerate(state["players"])]
    return "\n".join(lines)
...
async def cmd_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["status"] == "lobby":
        if len(state["players"]) >= MIN_PLAYERS and state["lobby_job"] is None:
            ...
            await update.message.reply_html(
                f"✅ {html.escape(user.first_name)} joined! Minimum players reached.\n\n"
                f"<b>Players ({len(state['players'])}):</b>\n{_player_list_text(state)}\n\n"
            ...
            await update.message.reply_html(
                f"✅ {html.escape(user.first_name)} joined the game!\n\n"
                f"<b>Players ({len(state['players'])}):</b>\n{_player_list_text(state)}"
            )
    else:
        # joined mid-game — appended to end of turn order
        await update.message.reply_html(
            f"✅ {html.escape(user.first_name)} joined mid-game and was added to the turn order!\n\n"
...
async def _post_turn(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"🎯 <b>{html.escape(current['name'])}</b>, choose your fate!\n\n"
        f"Next player: {html.escape(nxt['name'])}"
    )
...
async def tord_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"✅ {html.escape(current['name'])} has chosen {label}.", parse_mode="HTML"
    )
    ...
    await context.bot.send_message(
        chat_id,
        f"{header} for <b>{html.escape(current['name'])}</b>:\n\n\"{html.escape(prompt)}\"\n\n"
...
async def cmd_leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _remove_player(chat.id, context, idx, reason=f"{html.escape(user.first_name)} left the game.")
...
async def cmd_rm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _remove_player(
        chat.id, context, idx, reason=f"👢 {html.escape(target.first_name)} has been removed from the game."
    )
...
async def _remove_player(chat_id: int, context: ContextTypes.DEFAULT_TYPE, idx: int, reason: str):
    ...
    if len(state["players"]) < MIN_PLAYERS:
        del tord_games[chat_id]
        await context.bot.send_message(
            chat_id, f"{reason}\n\n🛑 Not enough players left, game cancelled!"
        )
        return
    ...
    if was_current_turn:
        await context.bot.send_message(chat_id, f"❕ {removed['name']} left mid-turn, skipping to next player.")
        await _post_turn(chat_id, context)
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🛑 The Truth or Dare game has been cancelled by {update.effective_user.first_name}."
    )
